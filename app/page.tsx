...
export default function Home(){
 ...
 return <>
  ...
  <main>
   ...
   <Dialog><div className="gallery">{works.map((work,index)=><article key={work.id} className={`artwork ${isWide(index)?'wide':''} ${index===1?'spotlight':''}`}><DialogTrigger className="art-trigger" onClick={()=>setActive(index)} aria-label={`查看插画：${work.title}`}><div className="image-frame"><img src={`/images/${work.id}-960.webp`} srcSet={sources(work)} sizes={index===1?'(max-width: 600px) calc(100vw - 40px), (max-width: 1100px) calc(100vw - 64px), (max-width: 1500px) calc(100vw - 112px), 1400px':isWide(index)?'(max-width: 600px) calc(100vw - 40px), (max-width: 900px) calc(100vw - 64px), (max-width: 1500px) 62vw, 880px':'(max-width: 600px) calc(100vw - 40px), (max-width: 900px) 44vw, (max-width: 1500px) 30vw, 430px'} width={work.width} height={work.height} alt={work.alt} loading={index<3?'eager':'lazy'} fetchPriority={index<2?'high':'auto'} decoding="async"/><span className="expand"><Plus size={20}/></span>{index<2&&<span className="featured">FEATURED / {String(index+1).padStart(2,'0')}</span>}</div><div className="caption"><span className="art-number">{String(index+1).padStart(2,'0')}</span><h3>{work.title}</h3><ArrowUpRight size={17}/></div></DialogTrigger></article>)}</div>
